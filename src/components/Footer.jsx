...
const Footer = () =>  (
    <section className={`${style.flexCenter} ${style.paddingY} flex-col`}>
                    ...
                    <div key={footerLink.key} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
                        ...
                        <ul className="list-none mt-4">
                            {footerLink.links.map((link, index) => (
                                <li key={link.name}
                                    className={`font-poppins font-normal text-[16px] leading-[12px] text-dimWhite hover:text-secondary cursor-pointer ${index !== footerLink.links.length - 1 ? 'mb-4' : 'mb-0'}`}>
                                    {link.name}
                                </li>
                            ))}
                        </ul>
        ...
        <div className="w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3F3E45]">
            <p className="font-poppins font-normal text-center leading-[27px] text-dimWhite">Copyright @ 2022 HooBank. All Rights Reserved.</p>
